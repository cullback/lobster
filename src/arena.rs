//! A small typed generational arena used by indexed book implementations.

use core::fmt;
use core::hash::{Hash, Hasher};
use core::marker::PhantomData;
use core::mem;
use core::num::NonZeroU32;

#[inline]
fn usize_index(index: u32) -> usize {
    usize::try_from(index).expect("arena index did not fit usize")
}

pub(crate) struct Key<Tag> {
    // One-based so Option<Key<Tag>> has the same layout as Key<Tag>.
    index: NonZeroU32,
    generation: u32,
    marker: PhantomData<fn() -> Tag>,
}

impl<Tag> Key<Tag> {
    fn new(index: u32, generation: u32) -> Self {
        let index = index
            .checked_add(1)
            .and_then(NonZeroU32::new)
            .expect("arena exhausted u32 keys");
        Self {
            index,
            generation,
            marker: PhantomData,
        }
    }

    const fn slot_index(self) -> u32 {
        self.index.get() - 1
    }
}

impl<Tag> Clone for Key<Tag> {
    fn clone(&self) -> Self {
        *self
    }
}

impl<Tag> Copy for Key<Tag> {}

impl<Tag> PartialEq for Key<Tag> {
    fn eq(&self, other: &Self) -> bool {
        self.index == other.index && self.generation == other.generation
    }
}

impl<Tag> Eq for Key<Tag> {}

impl<Tag> Hash for Key<Tag> {
    fn hash<HasherType: Hasher>(&self, state: &mut HasherType) {
        self.index.hash(state);
        self.generation.hash(state);
    }
}

impl<Tag> fmt::Debug for Key<Tag> {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        formatter
            .debug_tuple("Key")
            .field(&self.slot_index())
            .field(&self.generation)
            .finish()
    }
}

#[derive(Clone, Debug)]
enum Slot<T> {
    Occupied {
        generation: u32,
        value: T,
    },
    Vacant {
        generation: u32,
        next_free: Option<u32>,
    },
}

#[derive(Clone, Debug)]
pub(crate) struct Arena<T, Tag> {
    slots: Vec<Slot<T>>,
    free_head: Option<u32>,
    len: usize,
    marker: PhantomData<fn() -> Tag>,
}

impl<T, Tag> Arena<T, Tag> {
    pub(crate) const fn new() -> Self {
        Self {
            slots: Vec::new(),
            free_head: None,
            len: 0,
            marker: PhantomData,
        }
    }

    pub(crate) const fn len(&self) -> usize {
        self.len
    }

    #[cfg(test)]
    pub(crate) fn capacity(&self) -> usize {
        self.slots.len()
    }

    pub(crate) fn insert(&mut self, value: T) -> Key<Tag> {
        self.len += 1;
        if let Some(index) = self.free_head {
            let slot = &self.slots[usize_index(index)];
            let Slot::Vacant {
                generation,
                next_free,
            } = slot
            else {
                unreachable!("free-list entry was occupied");
            };
            let generation = *generation;
            self.free_head = *next_free;
            self.slots[usize_index(index)] = Slot::Occupied { generation, value };
            return Key::new(index, generation);
        }

        let index = u32::try_from(self.slots.len()).expect("arena exhausted u32 keys");
        self.slots.push(Slot::Occupied {
            generation: 0,
            value,
        });
        Key::new(index, 0)
    }

    pub(crate) fn get(&self, key: Key<Tag>) -> Option<&T> {
        match self.slots.get(usize_index(key.slot_index()))? {
            Slot::Occupied { generation, value } if *generation == key.generation => Some(value),
            Slot::Occupied { .. } | Slot::Vacant { .. } => None,
        }
    }

    pub(crate) fn get_mut(&mut self, key: Key<Tag>) -> Option<&mut T> {
        match self.slots.get_mut(usize_index(key.slot_index()))? {
            Slot::Occupied { generation, value } if *generation == key.generation => Some(value),
            Slot::Occupied { .. } | Slot::Vacant { .. } => None,
        }
    }

    pub(crate) fn remove(&mut self, key: Key<Tag>) -> Option<T> {
        let slot = self.slots.get(usize_index(key.slot_index()))?;
        let Slot::Occupied { generation, .. } = slot else {
            return None;
        };
        if *generation != key.generation {
            return None;
        }

        let next_generation = key.generation.wrapping_add(1);
        let old = mem::replace(
            &mut self.slots[usize_index(key.slot_index())],
            Slot::Vacant {
                generation: next_generation,
                next_free: self.free_head,
            },
        );
        self.free_head = Some(key.slot_index());
        self.len -= 1;

        let Slot::Occupied { value, .. } = old else {
            unreachable!("validated arena entry became vacant");
        };
        Some(value)
    }

    pub(crate) fn clear(&mut self) {
        self.slots.clear();
        self.free_head = None;
        self.len = 0;
    }
}

impl<T, Tag> Default for Arena<T, Tag> {
    fn default() -> Self {
        Self::new()
    }
}

#[cfg(test)]
mod tests {
    use core::mem::size_of;

    use super::{Arena, Key};

    enum TestTag {}

    #[test]
    fn key_uses_option_niche() {
        assert_eq!(size_of::<Option<Key<TestTag>>>(), size_of::<Key<TestTag>>());
    }

    #[test]
    fn rejects_a_key_after_its_slot_is_reused() {
        let mut arena = Arena::<_, TestTag>::new();
        let old_key = arena.insert("old");
        assert_eq!(arena.remove(old_key), Some("old"));

        let new_key = arena.insert("new");

        assert_eq!(arena.capacity(), 1);
        assert_ne!(old_key, new_key);
        assert_eq!(arena.get(old_key), None);
        assert_eq!(arena.get(new_key), Some(&"new"));
    }
}
